...
void DynamicGraphImpl::initialize(std::optional<ov::Tensor>& blob, NetworkMetadata& metadata) {
    _logger.info("Initialize dynamic graph execution");
    if (!_initializedMLIR) {
...
void DynamicGraphImpl::executeGraph(const std::shared_ptr<ZeroInitStructsHolder>& zeroInitStruct,
    ...
    if (args._impl == nullptr || !allTensorHasOldShape) {
        ...
        if (!noTensorChange) {
            ...
            if (npuMLIRRuntimeUpdateMutableCommandList(_engine,
                ...
                OPENVINO_THROW("Failed to execute MLIR runtime engine to update commandlist");
            }
    ...
    if (params->executionContext == nullptr) {
        if (npuMLIRRuntimeCreateExecutionContext(_engine, &params->executionContext) !=
            NPU_MLIR_RUNTIME_RESULT_SUCCESS) {
    ...
    if (params->executionContext) {
        _logger.debug("Execution context is created, try to set execution context for MLIR runtime engine");
    } else {
        _logger.debug("Execution context is not created, MLIR runtime engine will execute without execution context");
    }
