...
void DynamicGraph::execute(const std::shared_ptr<ZeroInitStructsHolder>& zeroInitStruct,
    ...
    auto mlirExecContext =
        reinterpret_cast<npu_mlir_runtime_execution_context_handle_t>(executionContext);
    if (impl == nullptr)
        return;

    impl->executeGraph(zeroInitStruct, args, commandLists, commandQueue, inferenceFence, event, profiling, mlirExecContext);
}
