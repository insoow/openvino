...
ZeroDynamicInferRequest::ZeroDynamicInferRequest(const std::shared_ptr<ZeroInitStructsHolder>& initStructs,
    ...
    for (const IODescriptor& outputDescriptor : _metadata.outputs) {
        ...
        _levelZeroOutputTensors.at(ioIndex) =
            allocate_tensor_for_pipeline(outputDescriptor, ioIndex, OUTPUT);
    ...
    if (_initStructs->getMutableCommandListExtVersion() >= ZE_MAKE_VERSION(1, 0) && batchSizeCandidate.has_value()) {
        ...
        for (size_t i = 0; i < tensors.size(); i++) {
            try {
                ...
                get_level_zero_input(foundPort.idx, i) =
                    allocate_tensor_for_pipeline(_metadata.inputs.at(foundPort.idx), foundPort.idx, INPUT, batchSizeCandidate);
            }
...
std::shared_ptr<ZeroTensor> ZeroDynamicInferRequest::allocate_tensor_for_pipeline(
    ...
    return allocate_tensor(descriptorWithUserInfo, index, isInput, std::nullopt);
}
...
std::shared_ptr<ZeroTensor>& ZeroDynamicInferRequest::get_level_zero_input(size_t index, size_t tensorNo) const {
